...
def _move_distance(player: str, source_text: str, target_text: str) -> int:
    if source_text == "bar":
        target = int(target_text)
        return target + 1 if player == "player_1" else POINTS - target
    source = int(source_text)
    if target_text == "off":
        return POINTS - source if player == "player_1" else source + 1
    target = int(target_text)
    return target - source if player == "player_1" else source - target
