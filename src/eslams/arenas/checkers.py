...
class CheckersArena(Arena):
    ...
    def apply_action(self, state: ArenaState, player_id: str, action: Any) -> ArenaState:
        ...
        board[er][ec] = _promote(piece, er)
        next_player = _other(player_id)
        outcome = _outcome(board, next_player)
        return self._state(
            board=board,
            turn=state.turn + 1,
            active=next_player,
    ...
    def _state(
        ...
        *,
        ...
        legal = [] if terminal else _legal_moves(board, active)
...
def _legal_moves(board: list[list[str | None]], player_id: str) -> list[str]:
    captures: list[str] = []
    moves: list[str] = []
...
def _outcome(board: list[list[str | None]], next_player: str) -> dict[str, Any] | None:
