...
6. **Separate machine slugs from display labels in validation summaries.** The latest server artifact validated successfully, but Platform still has to normalize values such as `verification_level = "Official Benchmark"` into machine keys. Core should emit a stable machine enum such as `official_benchmark` or `officialbenchmark` alongside any human display label so D1 projections, publication bundles, and audit code do not depend on string cleanup.
7. **Distinguish per-case scoring eligibility from aggregate leaderboard eligibility.** A one-case reviewed Trinity smoke can be valid, signed, and scoreable, while the public leaderboard must still require the locked full-suite sample count. Core official result and publication bundle contracts should expose this distinction directly so Platform can show a completed official proof run without implying it is a full public leaderboard ranking.
